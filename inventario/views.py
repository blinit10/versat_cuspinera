...
def actualizar_sistema(request):
    if DatosOrganizacion.objects.all()[0].test:
        ...
        response = requests.get('{}/backend/inventario/'.format(DatosOrganizacion.objects.all()[0].server),
                                headers=headers)
    json_products = response.json()
    actual_products = []
    proveedores = ProveedorPropio.objects.values_list('nombre')
    for json_product in json_products['data']:
        nombre = json_product['nombre_versat']
        if nombre == 'None':
            nombre = json_product['nombre']
        print(nombre)
        if (str(json_product['proveedor']['nombre']),) in proveedores:
            ...
            product = Producto(sku=json_product['sku'] or json_product['slug'],
                               nombre=nombre,
